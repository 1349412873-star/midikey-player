...
程序内置三套方案。方案名就是下表的三个名字。

| 方案 | 键位 | 音域 |
|---|---|---|
| 21 键自然音（默认） | 下排 `Z X C V B N M` = 中音 do..si；中排 `A S D F G H J` 高一个八度；上排 `Q W E R T Y U` 再高一个八度 | `1` ~ `7˙˙` |
| 21 键半音 | 同样三行七列：下排 `Z X C V B N M`、中排 `A S D F G H J`、上排 `Q W E R T Y U`。每列升一个半音 | `1` ~ `#5˙` |
| 36 键半音三排 | 低排 `, L . ; / I 9 O 0 P - [`（低八度）；中排 `Z S X D C V G B H N J M`（中音）；高排 `Q 2 W 3 E R 5 T 6 Y 7 U`（高八度）。每排 12 个半音 | `1.` ~ `7˙` |
...
后两套是半音阶：每个半音都有自己的键，一个音一个键，不用八度键与升半音键。

**功能键。** 「功能键」区块顶上有一个「启用功能键」勾选框。
勾上后，可以绑升高八度、降低八度、升半音三个键。音域随之向上下各扩一个八度。
不勾时该方案不用这三个键。超出键位范围的音直接不弹。

**改键位。** 点主界面的「键位设置…」按钮，打开独立的「键位设置」窗口。窗口分三块：
...
**方案文件。** 内置三套不能改名、不能删掉。想改就先点「新建方案…」复制一份。
...
当前 23 条断言全部通过，退出码 0。原来是 36 条：其中 13 条测的是自检内部的一份键名翻译副本，
副本已删，这 13 条一起删。

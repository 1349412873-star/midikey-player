...
**改键位。** 点主界面的「键位设置…」按钮，打开独立的「键位设置」窗口。窗口里可以改主键与音高、八度上键、八度下键、升半音键、基准音、音域上下限、超界策略、缺音策略。改动立即保存，关窗后卷帘与声轨颜色自动刷新。主界面只显示当前方案名，不放键位控件。

**超界策略。** 丢音 = 超出音域的音不发，卷帘上显示为灰色。就近折八度 = 把超出的音上下折到最近的合法八度再发。

**缺音策略。** 就近吸附 = 方案里没有的半音吸附到相邻自然音。丢弃 = 直接不发。

**键位预览图。** 蓝紫 = 有按键。橙 = 折八度后会响。灰 = 会丢音。红 = 音域内却没有按键。

**导入导出。** 「导入方案…」「导出方案…」走单个 JSON 文件。导入失败会给出中文提示，原设置不变。

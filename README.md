...
常用项就在右边。设备接入、输入兼容、热键、导出按键表、播放前自检这些不常改的，都搬进了独立的「高级设置」窗口，主界面只留一个按钮。
...
- **高级设置…**。点这个按钮打开独立窗口。设备接入、输入兼容、三个热键、导出按键表、播放前自检都在里面。改动即时生效，关掉窗口不影响设置。主界面因此不再堆这些项，卷帘拿到的高度也更多。
...
5. 跑内置自检，退出码必须是 0。
6. 跑主窗、键位窗与高级设置窗的界面快照，确认三个窗口都能出图。
   高级窗那一步会自己走一遍「开 → 关 → 再开」，顺带验证内容归属来回搬是干净的。

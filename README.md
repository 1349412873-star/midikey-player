...
- **播放后自动最小化窗口**。默认开。
- **导出按键表**。导出 G HUB 脚本（.lua），或通用 CSV。内容与实际演奏一致。
...
启动时的更新检查**默认关闭**。源码在 `MidiKeyPlayer/Engine/AutoUpdate.cs`。
要用它，先把 `Owner` 改成你的 GitHub 账号，再把 `Enabled` 改成 `true`。
仓库地址占位是 `https://github.com/<你的账号>/midikey-player`。

## 计划中

下面的功能还没实现。README 先留位置，实现后再补细节。

- **键位方案配置与导入导出**。自定义键位映射，可保存、导出、导入。
- **和弦开关**。允许同时按下多个音键。
- **声轨颜色**。给每条声轨指定颜色。

...
| 异环键位 | 键位同「21 键半音」。按 `Shift` 升半音，按 `Ctrl` 降低半音 | 低音 do 再低半音 ~ `7˙` | 开，`Shift` 升半音、`Ctrl` 降半音 |
...
当前 28 条断言全部通过，退出码 0。原来是 36 条：其中 13 条测的是自检内部的一份键名翻译副本，
副本已删，这 13 条一起删。

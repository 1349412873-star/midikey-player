...
**颜色。** 每条声轨一个唯一颜色。左侧列表的文字颜色与卷帘里的音符颜色一致。打击乐轨用灰色。未参与合奏的声轨降低不透明度。超出音域的音显示为灰色。
...
键位、音域、超界与缺音行为全部由方案文件决定，不写死在程序里。程序内置 6 套方案。

| 方案 | 键位 | 修饰键 | 音域 |
|---|---|---|---|
| 8 键单排（含高八度 do，默认） | `Z X C V B N M ,` = do..高音 do | 左键降八度、右键升八度、中键升半音 | 48..85 |
| 7 键单排自然音阶 | `Z X C V B N M` = do..si | 无 | 60..71 |
| 15 键三排 | `Q W E R T` / `A S D F G` / `Z X C V B` | 无 | 60..79 |
| 12 键半音阶双排 | `A W S E D F` / `T G Y H U J`，12 个半音一键一音 | `O` 升八度、`L` 降八度 | 48..83 |
| 5 键极简 | `1 2 3 4 5` = do re mi sol 高音 do | 无 | 60..72 |
| 宽音域 8 八度折叠 | 数字排加三排字母，36 键一键一音 | `PageUp` / `PageDown` 移八度、`Shift` 升半音 | 36..131 |
...
| 按键绑定 | 一行一条：选音高，点按键按钮，再按一次你的键就绑上 |
| 功能键 | 升高八度、降低八度、升半音，各自绑一个键 |
| 音域与取舍 | 能弹的音域，以及超出音域和缺音的处理方式 |

**等待按键。** 点按键按钮后它变成「按一个键…」，按一下键盘或鼠标就绑上。按 Esc 取消。`PageUp`、鼠标键这类手按不到的，点旁边的「选键名」从列表里选。

**哪些音还没绑。** 窗口底部常驻一行统计，例如「绑了 8 条。能弹 C3 到 C#6，其中 30 个音还没有绑键。」

**超界策略。** 「不弹这个音」= 超出音域的音不发。 「挪到最近的八度再弹」= 把音上下折到最近的合法八度再发。

**缺音策略。** 「用最近的音代替」= 方案里没有的半音用相邻音顶上。 「跳过这个音」= 直接不发。

**方案文件。** 内置 6 套方案不能改。想改就先「另存为」一份。自定义方案在 `%LOCALAPPDATA%\MidiKeyPlayer\schemes\` 下，重启后仍在列表里。「导入方案…」「导出方案…」走单个 JSON 文件，方便互相分享。

...
#
# 产物与线上 Release 一致：zip 根目录下只有 MidiKeyPlayer.exe 与「更新说明.txt」。
...
"$DOTNET" publish MidiKeyPlayer.csproj -c Release \
